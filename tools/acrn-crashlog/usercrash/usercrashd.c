...
/*Usercrash works in Client/Server model: usercrashd works as server, which is
 * to detect events from client in endless loop. Once server receives events
 * from client, it will create usercrash_0x file under /var/log/usercrashed/
...
{
	printf("usercrashd - tool to dump crash info for the crashes in the ");
	printf("userspace on sos. It's the server role of usercrash.\n");
	printf("[Usage] usercrashd (root role to run)\n");
	printf("[Option]\n");
	printf("\t-h: print this usage message\n");
	printf("\t-v: print usercrashd version\n");
...
int main(int argc, char *argv[])
{
	...
	if (argc > 1) {
		while ((opt = getopt(argc, argv, "vh")) != -1) {
			switch (opt) {
			case 'v':
				printf("usercrashd version is 1.0\n");
				break;
			case 'h':
				print_usage();
				break;
